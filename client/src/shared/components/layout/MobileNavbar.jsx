...
export default function MobileNavbar({ isAdminLayout }) {
    ...
    return (
        <>
            <div
                className={`md:hidden rydo-safe-top sticky top-0 shrink-0 w-full border-b border-border bg-black/40 backdrop-blur-xl ${isOpen ? 'z-(--rydo-z-mobile-menu)' : 'z-(--rydo-z-sticky)'}`}
            >
                ...
                <AnimatePresence>
                    {isOpen && (
                        <MotionDiv
                            ...
                            exit={{ opacity: 0 }}
                            transition={tFast}
                            className={`absolute top-full left-0 w-full max-h-[calc(100dvh-4.5rem)] overflow-y-auto overscroll-contain border-b border-border p-4 pb-[max(1rem,var(--rydo-safe-bottom))] flex flex-col gap-2 shadow-[0_24px_48px_rgba(0,0,0,0.45)] ${isAdminLayout ? 'bg-[var(--rydo-bg-deep)]' : 'bg-[var(--rydo-bg-deep)]'}`}
                        >
